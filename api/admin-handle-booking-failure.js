...
async function getAdminProfile({ supabaseUrl, serviceRole, userId }) {
  ...
  if (!r.ok) throw new Error(`Profile lookup failed: ${r.status} ${r.text}`);
...
function statusForAction(action) {
  ...
  if (["new", "in_review", "reviewed", "resolved", "closed"].includes(a)) {
    return a;
  }
  ...
  try {
    ...
    if (profile?.role !== "admin") {
      return res.status(403).json({
        ok: false,
        error: "Only admins can update booking alerts.",
      });
    }

    const id = String(req.body?.id || "").trim();
    const newStatus = statusForAction(req.body?.action);
    ...
    const url = new URL(`${SUPABASE_URL}/rest/v1/booking_failure_events`);
    ...
    if (!r.ok) {
      return res.status(500).json({
        ok: false,
        error: "Could not update booking alert.",
        details: r.data,
      });
    }
    ...
    return res.status(200).json({
      ok: true,
      event: Array.isArray(r.data) ? r.data[0] || null : null,
    });
  } catch (err) {
    return res.status(500).json({
      ok: false,
