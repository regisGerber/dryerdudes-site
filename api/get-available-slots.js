...
//
// UPDATE (2026-02-18):// /api/get-available-slots.js
...
module.exports = async (req, res) => {
  try {
    ...
    // Build "YYYY-MM-DDTHH:MM:SS-08:00" for comparisons (use LOCAL_TZ_OFFSET)
    const LOCAL_TZ_OFFSET = String(process.env.LOCAL_TZ_OFFSET || "-08:00");
    const makeLocalTimestamptz = (service_date, hhmmss, offset = LOCAL_TZ_OFFSET) => {
    ...
    const PRI_OPT4_ADJ_PM = [8, 7]; // adjacent PM inside-out
    ...
    if (!ztaResp.ok) {
      return res.status(500).json({
        ...
        body: ztaText.slice(0, 500),
      });
    }

    const homeTechId = Array.isArray(ztaRows) && ztaRows[0]?.tech_id ? String(ztaRows[0].tech_id) : null;

    // If no tech mapping, safest is: return no slots (your "nothing should book without a tech" rule).
    if (!homeTechId) {
      return res.status(200).json({
        ...
        meta: debug
          ? { debug: true, nowLocal, todayISO, horizonISO, note: "No tech assigned for this zone (zone_tech_assignments missing)." }
          : undefined,
      });
    }
...
if (!offResp.ok) {
  return res.status(500).json({
    ...
    const isSlotOffForHomeTech = (service_date, start_time, end_time) => {
      ...
      const eIso = makeLocalTimestamptz(service_date, end_time);
    ...
    /* -------------------- NEW: filter candidates by HOME tech time off -------------------- */
    // Only remove slots that would be worked by the HOME zone's tech.
    // In your current model (one tech owns A-D), this removes the slot globally for that request.
    offerCandidates = offerCandidates.filter((r) => {
      // If home tech is off for this window, do not offer it.
      // We use the slot's service_date + start/end_time.
      ...
      return res.status(200).json({
        ...
        meta: debug
          ? {
              ...
              note: "All candidates removed due to time-off filtering for the zone's assigned tech.",
            }
          : undefined,
      });
    }
    ...
    return res.status(200).json({
      ...
      meta: debug
        ? {
            ...
            note:
              "Option B active. bookings is truth. horizon 21 days. Added filter: remove slots during HOME zone's assigned tech time off (slot=exact, am/pm/all_day=overlap).",
          }
        : undefined,
    });
  } catch (err) {
